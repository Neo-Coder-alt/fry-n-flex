...
import { X, Plus, Minus, Trash2, ShoppingBag, ArrowRight, CheckCircle2, Loader2, ArrowLeft } from 'lucide-react';
          ...
          {stage === 'checkout' && (
            <form id="checkout-form" onSubmit={handleCheckout} className="px-5 py-5 space-y-4">
              ...
              <p className="text-xs text-white/40">
                Payment is cash on delivery. We'll call to confirm your order.
              </p>
            </form>
          )}
          ...
          {stage === 'done' && (
            <div className="flex flex-col items-center justify-center h-full px-8 text-center gap-4">
              ...
              <p className="text-white/60 text-sm max-w-xs">
                Your order has been placed. We'll call <span className="text-white/85 font-medium">{phone || 'you'}</span> shortly to confirm.
              </p>
              <button
                onClick={close}
                className="mt-2 inline-flex items-center gap-2 px-6 py-3 rounded-full bg-gold-400 text-ink-950 font-bold hover:bg-gold-300 transition-colors"
              >
                Back to menu
              </button>
            </div>
